...
int main() {
	...
	char *attr_buffer = create_attributes(attrs, sizeof(attrs)/sizeof(attrs[0]), &attr_buffer_len);

	xpc_object_t msg = xpc_dictionary_create(NULL, NULL, 0);
	xpc_dictionary_set_uint64(msg, SYSMON_XPC_KEY_TYPE, SYSMON_MSG_TYPE_PROCESS_REQUEST);
	xpc_dictionary_set_uint64(msg, SYSMON_XPC_KEY_FLAGS, 0);
	xpc_dictionary_set_data(msg, SYSMON_XPC_KEY_ATTRIBUTES, attr_buffer, attr_buffer_len);

	xpc_connection_t conn = xpc_connection_create_mach_service(SYSMON_XPC_SERVICE_NAME, NULL, 0);
	if (conn == NULL) {
